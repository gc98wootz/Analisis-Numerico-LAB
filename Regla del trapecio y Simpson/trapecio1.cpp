...
float Simpson3_8(float a,float b,int n,float (*funcion)(float)){
  ...
  for(int i=0;i<n/3;i++){
    //if(i%3)
      S0+=2*funcion(a+(i+2)*h);
    //else
      S1+=3*funcion(a+(i+1)*h);
      S2+=3*funcion(a+i*h);
  }
  //cout<<S0<<endl;
  //cout<<S1<<endl;
  sumatoria=(((3*h)/8)*(funcion(a)+S0+S1+S2+funcion(b)))*-1;
  //cout<<sumatoria<<endl;
  return sumatoria;
}
...
int main(){
  ...
  for(int i=6;i<36;i+=6){
    cout<<"\t"<<i<<"\t\t"<<trapecio(1,0,i)<<"\t\t"<<Simpson1_3(1,0,i)<<"\t\t"<<Simpson3_8(1,0,i,funcion)<<endl;
  }
